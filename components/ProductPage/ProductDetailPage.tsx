...
import Link from "next/link";
import { useRouter } from "next/navigation";
import { ChevronRight, Heart, ShoppingCart, Loader2, AlertCircle } from "lucide-react";
import { getProductBySlug } from "@/lib/supabase/products";
import { supabase } from "@/lib/supabase/client";
              ...
              {product.variants && product.variants.length > 0 && (
                <div 
                  ...
                  className={`border-t border-white/10 pt-6 transition-all duration-300 ${
                    showVariantError ? 'ring-2 ring-red-500/50 rounded-lg p-4 -m-4' : ''
                  }`}
                >
                    ...
                    {product.variants.map((variant, index) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            backgroundColor: isSelected ? GOLD : "rgba(255,255,255,0.05)",
                            borderColor: isSelected ? "transparent" : showVariantError ? "rgba(239, 68, 68, 0.5)" : "rgba(255,255,255,0.10)",
                            color: isSelected ? "#000000" : "rgba(255,255,255,0.85)",
                          }}
                        >
                          {variantLabel}
